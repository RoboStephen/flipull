...
class BoardState:
    ...
    def load(cls) -> BoardState:
        ...
        # K = key 7
        letters = ".sotxWXK"
        with Path("castle.txt").open("r") as in_file:
            header_line = in_file.readline().strip()
            _level_number, column_digits, row_digits = header_line.split()
            ...
            for row in range(len(row_digits)):
                ...
                row += 1
        return BoardState(moves=[], board=board_grid, 
    ...
    def clone(self) -> BoardState:
        ...
        new_board = BoardState(moves=new_moves, board=new_board, attack_columns=self.attack_columns, 
                               attack_rows=self.attack_rows, score=self.score, held_block=self.held_block)
        return new_board
    ...
    def is_legal_move(self, is_column: bool, move_position: int) -> tuple[bool, bool]:
        ...
        while True:
            ...
            #print(f"is_legal_move({is_column}, {move_position}) - at {row},{column} see {current_block} dx={dx} dy={dy}")
            # If we hit a wall, turn:
            if current_block == BlockType.wall:
                if dy == 1:
                    return False, False
                column -= 1
                row += 1
            ...
            if current_block != 0:
                ...
                if self.held_block == 0 or current_block in (self.held_block, BlockType.key, BlockType.wild):
    ...
    def make_move(self, is_column: bool, move_position: int) -> bool:
        ...
        while True:
            ...
            if current_block == BlockType.wall:
                if dy == 1:
                    if blocks_cleared == 0:
                        print("* Illegal: Hit wall from above")
                        return False
                    else:
                ...
                dy = 1
                if row >= row_count:
                    print("* Illegal: Hit wall and turned off the edge")
                    return False
                continue
            if current_block != 0:
                if self.held_block != 0 and self.held_block != current_block:
                    if current_block not in (BlockType.key, BlockType.wild) and blocks_cleared == 0:
                    ...
                    if swap == BlockType.wild:
                        #self.held_block = 0
                        self.score += 1000
                    else:
                        self.held_block = swap
                        #self.score += 100 + (blocks_cleared * 100)
                        return True
                ...
                for above_row in range(row-1, -1, -1):
                    if self.board[above_row, column] == BlockType.wall:
                        found_wall = True
                        self.board[above_row+1, column] = 0
                        break
...
class MoveChecker:
    ...
    def try_move(self, board: BoardState):
        ...
        for (is_column, move_position) in try_moves:
            ...
            if new_board.held_block != BlockType.key:
...
class Flipull:
    def main(self):
        ...
        job_list = []
        for row in board.attack_rows:
            if board.is_legal_move(False, row):
                new_board = board.clone()
                job_list.append((new_board, False, row))
        for column in board.attack_columns:
            if board.is_legal_move(True, column):
                new_board = board.clone()
